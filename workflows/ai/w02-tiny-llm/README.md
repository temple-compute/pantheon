...
this repository. `prepare_tinystories_{train,val}` take `repo_dir` as an input purely to give
every task in this workflow one shared root (`clone_repo`), even though the tokenizer script
itself doesn't read from the checkout — `horus run` with no `--trigger` defaults to the first
task in `tasks:` and only schedules that task's downstream closure, so a task with no edge back
to the root would silently never run.
...
- No instruction tuning, eval, or alignment stages — see [W-28](../w01-train-llm/README.md) for
  a full pretrain → SFT → eval pipeline (on the Pile, not TinyStories).
- Multi-GPU (`torchrun`) execution isn't wired into any task; this workflow assumes single
  GPU/CPU local execution, which is also all a tiny run like this needs.
